...
class _MapScreenState extends State<MapScreen> with GoogleMapMixin {
      ...
      if (pendingOrders.isEmpty) {
        return Align(
          ...
          child: Container(
            ...
            child: Text(
              S.of(context).noPendingDeliveryOrder,
    ...
    for (var element in orders) {
      if (element.deliveryStatus == DeliveryStatus.pending) {
        final index = stores.indexWhere(
          (store) => store.id == element.store?.id,
        );
        if (index == -1 && element.store != null) {
          stores.add(element.store);
        }
      }
    }
  ...
  void _moveToCard({Store? store, Order? order}) {
    if (order != null) {
      try {
        locationFromAddress(DeliveryTools.formatAddress(order.shipping!)).then((
          coordinates,
        ) {
                      ...
                      Expanded(
                        child: InfoWidget(
                          ...
                          onCallBack: store.phone == null
                              ...
                              : store.phone!.trim().isNotEmpty
                              ? () async {
                                  await Tools.launchURL('tel:${store.phone}');
                                }
                              : null,
                        ),
                      ),

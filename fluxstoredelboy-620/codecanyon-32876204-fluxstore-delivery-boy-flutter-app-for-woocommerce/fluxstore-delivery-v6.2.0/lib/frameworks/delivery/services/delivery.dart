...
class DeliveryService extends WooCommerceService {
  ...
  Future<User?> login({username, password}) async {
    ...
    try {
      final response = await _connector.httpPost(
        '$domain/wp-json/api/flutter_user/generate_auth_cookie/?insecure=cool'
            .toUri()!.addUrlQuery(isSecure),
      ...
      if (response.body.isEmpty) {
        throw Exception('Server error (${response.statusCode}): Empty response. Check if MStore API plugin is active and Permalinks are set to "Post name".');
      }
  ...
  Future<User?> createUser({
    ...
    try {
      ...
      final response = await _connector.httpPost(
        '$domain/wp-json/api/flutter_user/sign_up/?insecure=cool'
            .toUri()!.addUrlQuery(isSecure),
        body: convert.jsonEncode({

...
class _LoginWidgetState extends State<LoginWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final model = Provider.of<DeliveryAuthenticationModel>(
      context,
      listen: false,
    );
          ...
          children: [
            ...
            InkWell(
              onTap: () async {
                ...
                if (await context.checkInternet()) {
                  unawaited(model.login(widget.onMessage));
                }
              },
            ...
            SocialLoginButtonRow(
              onApplePressed: () async {
                if (await context.checkInternet()) {
                  unawaited(model.appleLogin(widget.onMessage));
                }
              },
              onFacebookPressed: () async {
                if (await context.checkInternet()) {
                  unawaited(model.facebookLogin(widget.onMessage));
                }
              },
              onGooglePressed: () async {
                if (await context.checkInternet()) {
                  unawaited(model.googleLogin(widget.onMessage));
                }
              },
              onSmsPressed: () async {
                if (await context.checkInternet()) {

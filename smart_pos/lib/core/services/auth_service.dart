...
class AuthService extends ChangeNotifier {
  ...
  Future<bool> signIn({
    ...
    try {
      // If Firebase is not initialized, try offline directly
      if (_auth == null) {
        return await signInOffline(email: email, password: password);
      }
      ...
      // Fallback to offline sign-in if Firebase fails (e.g. wrong config or network issue)
      debugPrint('Firebase sign in failed: ${e.code}. Trying offline...');
      final offlineSuccess = await signInOffline(email: email, password: password);
